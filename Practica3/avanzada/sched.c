	...
	while (!feof(file) && fgets(line, MAX_CHARS_TASK_LINE, file)) {
		...
		if (strcmp(line, "\n")) {
			...
			while (!error && (token = strsep(&parsed_line, " ")) != NULL ) {
				...
				case 2:
					if (sscanf(token, "%d",
							&cur_task->task_profile.arrival_time) != 1)
						error = 1;
					break;
				default:
					phase =
							&cur_task->task_profile.phase[cur_task->task_profile.nr_phases];
			...
			if (cur_task->task_profile.nr_phases == 0) {
				fprintf(stderr,
						"Error parsing CPU/IO pattern for task (line %d) -> %s \n",
						line_counter, line);
				exit(1);
			}
...
static void add_log_register(task_t* task, task_state_t state, int cpu,
		int when, int how_long) {
...
static void print_task_log_registers(task_t* task) {
	...
	while (!is_empty_slist(&task->sched_regs)) {
		write_cpu_log(cur->cpu, "%s\t%d\t%d\t%s\n", task->task_name, cur->when,
				cur->when + cur->how_long, strstate[cur->state]);
	...
	while ((event = get_next_sched_event(cpu, simulation_step))) {
		...
		case NEW_TASK:
			...
			if (exec_phase->type == IO_PHASE) {
				fprintf(stderr,
						"Critical error: IO Phase detected when awaking task of a task's profile\n");
				sched_terminate(1);
			}
	...
	if (!is_idle_task(current) && (--current->runnable_ticks_left) == 0) {
		...
		if (task_profile->cur_phase >= task_profile->nr_phases) {
			...
			if (exec_phase->type == CPU_PHASE) {
				fprintf(stderr,
						"Critical error: CPU Phase detected when leaving the CPU\n");
			...
			add_log_register(current, TS_SLEEP, cpu, simulation_step + 1,
					exec_phase->len);
			write_cpu_log(cpu, "(t%d): Task %s goes to sleep until (t%d)\n",
					simulation_step + 1, current->task_name,
					simulation_step + exec_phase->len + 1);
		}
	}
 ...
 */
static void move_one_task(runqueue_t* src_rq, int src_cpu, runqueue_t* dst_rq,
		int dst_cpu, int this_cpu, int simulation_step) {
	...
	add_log_register(stolen_task, TS_RUNNABLE, src_cpu,
			stolen_task->last_time_enqueued, time_on_rq);
	...
	while (!load_balanced_system && (nr_trials < max_trials)) {
		...
		if (max_load - min_load <= 1) {
			...
			if ((this_rq->nr_runnable - remote_rq->nr_runnable) > 1)
				move_one_task(this_rq, this_cpu, remote_rq, cpu_min_load,
						this_cpu, simulation_step);
			...
			if ((remote_rq->nr_runnable - this_rq->nr_runnable) > 1)
				move_one_task(remote_rq, cpu_max_load, this_rq, this_cpu,
						this_cpu, simulation_step);
...
static void schedule(int cpu, int simulation_step) {
	...
	if (cpu_rq->need_resched || is_idle_task(prev)) {
		...
		if (prev != next) {
			...
			/* Keep track of how long the task was runnable */
			add_log_register(next, TS_RUNNABLE, cpu, next->last_time_enqueued,
					simulation_step - next->last_time_enqueued + 1);
...
static inline int cpu_has_completed(int cpu) {
	return (get_runqueue_cpu(cpu)->nr_runnable == 0
			&& is_empty_slist(&sched_events[cpu]));
}
...
static inline void insert_sched_event(sched_event_t* event, int cpu) {
	sorted_insert_slist(&sched_events[cpu], event, 1,
			compare_sched_event_timeouts);
}
