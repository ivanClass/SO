...
static void enqueue_task_sjf(task_t* t, int cpu, int runnable) {
	...
	if (!runnable) {
		...
		if (preemptive_scheduler && !is_idle_task(current)
				&& t->runnable_ticks_left < current->runnable_ticks_left) {
...
sched_class_t sjf_sched = { .pick_next_task = pick_next_task_sjf,
		.enqueue_task = enqueue_task_sjf, .task_tick = task_tick_sjf,
