...
int resizeNode(uint64_t idxNode, size_t newSize) {
	...
	memset(block, 0, sizeof(char)*BLOCK_SIZE_BYTES); //Inicializa a 0 todas las posiciones de block[]
	...
	/// File decreases
		...
		int numBlocks = (newSize + BLOCK_SIZE_BYTES - 1) / BLOCK_SIZE_BYTES; //por si se modifican trozos de un archivo (si se elimina un archivo, esto va a dar 0
		myFileSystem.superBlock.numOfFreeBlocks += (node->numBlocks - numBlocks); //si se elimina un archivo, los bloques que estaban ocupados se suman para ponerlos a libre
		...
		for(i = node->numBlocks; i > numBlocks; i--) {
			...
			if((lseek(myFileSystem.fdVirtualDisk, nBloque * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) || //comprueba que el puntero, tras el write, apunte donde tenga que apuntar
			        (write(myFileSystem.fdVirtualDisk, &block, BLOCK_SIZE_BYTES) == -1)) { //pone el block lleno de ceros en el bloque borrado
				perror("Failed lseek/write in resizeNode");
				return -EIO;
			}
		}
...
static int my_unlink(char *path){
	...
	if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) == -1) { //+1 para no sararle /nombre sino nombre (sin /)
		return -ENOENT;
	}
	...
	NodeStruct *nodeAux = myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx];
	nodeAux->freeNode = true;

	myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx] = NULL;  //LO PONEMOS A NULL PARA QUE LA FUNCION FindFreeNode ENCUENTRE
																			  //NODOS VACIOS (LA FUNCION NO MIRA SI EL NODO ESTA LIBRE O NO
	updateDirectory(&myFileSystem);
	updateNode(&myFileSystem, myFileSystem.directory.files[idxDir].nodeIdx, nodeAux);
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	while(bytes2Read) {
		...
		for(i = offBloque; (i < BLOCK_SIZE_BYTES) && (totalRead < size); i++) {
			buf[totalRead++] = buffer[i]; //Primero lee totalRead y luego imcrementa
		}

		// Discont the read stuff
...
static int my_rename(const char *nombreViejo, const char *nombreNuevo){
	int idxDir;
	int idxDir2;
	int i = 0;
	char lee;
	...
	if((idxDir = findFileByName(&myFileSystem, (char *)nombreViejo + 1)) == -1) { //+1 para no sararle /nombre sino nombre (sin /)
		return -ENOENT;
	}

	if((idxDir2 = findFileByName(&myFileSystem, (char *)nombreNuevo + 1)) != -1) { //+1 para no sararle /nombre sino nombre (sin /)
		fprintf(stdout,"Realmente desea eliminar %s (y/n) ?", (char *)nombreNuevo + 1); //MIRAR ESTO
		fscanf(stdin,"%c",&lee);
		if(lee != 'y' && lee != 'n'){
			fprintf(stdout,"Operacion cancelada");
			return -1;
		}
		if(lee == 'n'){
			fprintf(stdout,"Operacion cancelada");
	...
	//CREEMOS QUE ESTE BUCLE PUEDE NO HACER FALTA
	for(i = 0; i < 16;i++){
		myFileSystem.directory.files[idxDir].fileName[i] = aux[i];
	}

	aux = (char *)nombreNuevo +1;
...
struct fuse_operations myFS_operations = {
	...
	.rename		= my_rename,					// Cambia el nombre de un archivo
	.chmod		= my_chmod,						// Cambia los permisos de un fichero
};

//COMANDO MONTAR
// -t 2097152 -a disco-virtual -f '-d -s punto-montaje'

//COMANDO DESMONTAR
// fusermount -u punto-montaje (se tiene que estar en la ruta donde está)
